    ...
    setup() {
        ...
        onMounted(() => {
            workspace.value.workspace.addChangeListener(() => {
                xml.value =  Blockly.Xml.domToPrettyText(Blockly.Xml.workspaceToDom(workspace.value.workspace))
                pythonCode.value =  Blockly.Python.workspaceToCode(workspace.value.workspace)
            })
        })
